...
for m=1:size(zvec,1)
    indmin=find(VZ==zvec(m,1));thicknessVY
...
inpFilename=[strrep(filename,'.geo','_automesh.geo'),'.inp'];
fid=fopen(inpFilename,'w+');

lambda=1.55;

dx=subGridMultiLayer(epsVX,thicknessVX,lambda);
dy=subGridMultiLayer(epsVY,thicknessVY,lambda);
dz=subGridMultiLayer(epsVZ,thicknessVZ,lambda);

writeMesh(dx,dy,dz,nx,ny,nz,fid);
fclose(fid)
