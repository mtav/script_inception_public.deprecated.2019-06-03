function wavesimulator_1d(thickness_vector,n_vector,medium_color_vector,wave_color_vector,lambda,signal_origin,object_origin,x_range,n_outside)
  ...
  if exist('medium_color_vector','var') == 0; medium_color_vector = {[0,0,n_vector(1)/2.4],[0,0,n_vector(2)/2.4],[0,0,n_vector(3)/2.4],[0,0,n_vector(4)/2.4]}; end;
  if exist('wave_color_vector','var') == 0; wave_color_vector = {[0,0,0],[1,0,0],[0,1,0],[0,0,1]}; end;
  ...
  for i = 1:N
    ...
    if (i>1)
      ...
      amplitude(i) = amplitude(i-1)*T(i)
    end
  end
  x(N+1) = x(N) + thickness_vector(N);

  for i = 1:N
    linspace(x(i),x(i+1),10)
    transmitted_wave_x{i} = linspace(x(i),x(i+1),100);
    transmitted_wave_y{i} = amplitude(i)*sin(k(i)*transmitted_wave_x{i} + phase(i));
  end
