...
function filename_cellarray = multiLine(folder,mag,prefix,direction,interRingDistancePxl)
  % ex: b=multiLine('TESTDIR/str_line',36000,'line',1,1)
...
%  r=[h+w*6+c*5+t*15];
  ...
  disp(['R_outer_pxl = ',num2str(R_outer_pxl)]);
  ymin_pxl = round(ymin_mum/res);
  ymax_pxl = round(ymax_mum/res);
  % height_pxl = round(height_mum/res)
  height_pxl = ymax_pxl - ymin_pxl + 1;
  ring_Width = 2.1; %um
  ...
  for idx=1:length(R)
    ...
    rvec_new = 1:Rad_pxl;
    rad = repmat(rvec_new,pxPerLine,1);
    ...
    dwell = repmat(dwell,pxPerLine,1);
    dwell = dwell(:)';

    if pxPerLine*nnz(profile)>1e6
        %size(linspace(0,2*pi,pxPerLine))
        size(Rad_pxl)
        pxPerLine
        Rad_pxl
        nnz(profile)
        pxPerLine*nnz(profile)
        disp('WARNING: NOT ENOUGH MEMORY. Exiting');
        return
    end
    %theta = repmat(linspace(0,1,2*pxPerLine),1,Rad_pxl);
    %theta = repmat(linspace(0,2*pi,pxPerRing),1,Rad_pxl);
    ypoints = repmat(linspace(ymin_pxl,ymax_pxl,pxPerLine),1,Rad_pxl);
    ...
    x = [2048-x,2048+x];%-round((min(x)+max(x))/2);
