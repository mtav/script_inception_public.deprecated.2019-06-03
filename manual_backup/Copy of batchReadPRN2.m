...
% ex: batchReadPRN('pillar_M2754_0_10', [62,71,80,89], 3, 2)
%
% invalid: batchReadPRN(folder,infilename, 3, 2)
% invalid: batchReadPRN('pillar_M2754_0_10', ['p62id.prn','p71id.prn','p80id.prn','p89id.prn'], 3, 2)
%
% snapshot_col:
%1 x
%2 z
...
for m=1:length(id_list)
    ...
    %calculate power of fft
    c_y_pow = cFFT_output.* conj(cFFT_output)/length(cFFT_output);
	...
	% lambda(2)%2000
	% lambda(1)%Inf
	% lambda(length(lambda))-lambda(1)
	
    b_wavelength=1e3*b_lambda(b_indMin:b_indMax); %Unit of wavelength is nm.
	b_wavelength=b_wavelength';
	
    b_Mag_zoom_1 = b_Mag(indMin:indMax);
	
    figure(3);hold off;
    plot(b_wavelength,b_Mag_zoom_1);
	hold on;
    plot(c_wavelength,c_Mag_zoom_1);
	...
	disp('	search for peaks')
	% peaks = peakdet(Mag, delta/3);
    % [A,B] = peakdet(Mag, delta/3);
	% A
	% size(A)
	% B
	% size(B)
    ...
    b_wavelength = 1e3*b_lambda(indMin:indMax); %Unit of wavelength is nm. (lambda is in mum)
	b_wavelength = b_wavelength';
	
    b_Mag_zoom_2 = b_Mag(b_indMin:b_indMax);
	
    figure(4);hold off;
    plot(b_wavelength,b_Mag_zoom_2);
	hold on;
    plot(c_wavelength,c_Mag_zoom_2);
