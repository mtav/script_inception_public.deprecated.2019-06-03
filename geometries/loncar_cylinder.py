...
def loncar_cylinder(BASENAME = 'loncar_structure', DSTDIR = getuserdir(), ITERATIONS=32000, print_holes_top=True, print_holes_bottom=True, HOLE_TYPE=1, pillar_radius_mum = 0.150/2.0, EXCITATION_FREQUENCY = get_c0()/637e-3, SNAPSHOTS_FREQUENCY=[],excitation_type=1):
  ...
  # for probes
  probes_X_vector = Xplanes[1:length(Xplanes)-1]
  ...
  # open file
  out = fopen(DSTDIR+os.sep+BASENAME+os.sep+BASENAME+'.geo','wt')
  ...
  # open file
  out = fopen(DSTDIR+os.sep+BASENAME+os.sep+BASENAME+'.inp','wt')
...
def main(argv=None):
  ...
  try:
    ...
    # more code, unchanged
    print('hello')
    loncar_cylinder('test_python', os.getenv('TESTDIR'), 32000, 1, 1, 1, 0.150/2.0, get_c0()/0.637, [get_c0()/0.637, get_c0()/0.637-1, get_c0()/0.637+1],1)
