function [ vEnd, vStart, dt, fmin, fmax, peak_frequency_vector, data_min, data_max ] = analyzePRN(fullpath, peak_file, delta_scaling, snapshot_col, probe_col)
	...
	if freq_plot_all == 1
		...
		% hdrsave(   [filebasename,'_',header{probe_col},'_FFT_all','.prn'],'wavelength Mag',[ wavelength_vec(:), c_Mag(:) ]);
	end
	...
	if freq_plot_zoom1 == 1
		...
		hdrsave(   [filebasename,'_',header{probe_col},'_FFT_zoom1','.prn'],'wavelength Mag',[ wavelength_vec(:), c_Mag_zoom_1(:) ]);
	end
	...
	if freq_plot_zoom2 == 1
		...
		hdrsave(   [filebasename,'_',header{probe_col},'_FFT_zoom2','.prn'],'wavelength Mag',[ wavelength_vec(:), c_Mag_zoom_2(:) ]);
	end
